...
Ollama hosts CPU-first local open-weight models in Kubernetes. The active model-scoped instance is `ollama-qwen-coder` with `qwen2.5-coder:7b`; `ollama-deepseek-r1` and `ollama-gemma3` are defined but scaled to zero. 9Router remains the primary gateway.
